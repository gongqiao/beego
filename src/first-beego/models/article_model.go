...
type Article struct {
	Id                                  int
	Title, Tags, Short, Content, Author string
	CreateTime                          int64
}
...
//根据页码查询文章
func FindArticleByPage(page int) ([]Article, error) {
	//从配置文件中获取每页的文章数量
...
func QueryArticleByTag(tag string) ([]Article, error) {
	sql := " where tags like '%s" + tag + "&%'"
...
func QueryArticleById(id int) Article {
	sql := fmt.Sprintf("select id,title,tags,short,content,author,createtime from article where id = %d", id)
	row := utils.QueryRowDB(sql)
	article := Article{}
	row.Scan(&article.Id, &article.Title, &article.Tags, &article.Short, &article.Content, &article.Author, &article.CreateTime)
	//art := Article{id,title,tags,short,content,author,createtime}
	return article
}